 ...
 */
async function resolveCatalogsAndServices(rawCatalogs) {
    const uniqueCatalogs = Array.from(new Set(rawCatalogs || []));
    if (!uniqueCatalogs.length) {
        throw new ApiError(StatusCodes.BAD_REQUEST, 'Debes enviar al menos un catálogo');
    }
    // Validar que sean ObjectId válidos
    const invalid = uniqueCatalogs.filter(id => !mongoose.isValidObjectId(id));
    if (invalid.length) {
        throw new ApiError(StatusCodes.BAD_REQUEST, `IDs de catálogo inválidos: ${invalid.join(', ')}`);
    }
    ...
    if (catalogs.length !== uniqueCatalogs.length) {
        throw new ApiError(StatusCodes.BAD_REQUEST, 'Uno o más catálogos no existen o están inactivos');
    }
    ...
    if (!allServiceIds.length) {
        throw new ApiError(StatusCodes.BAD_REQUEST, 'Los catálogos seleccionados no tienen servicios asociados');
    }
    ...
    if (count !== allServiceIds.length) {
        throw new ApiError(StatusCodes.BAD_REQUEST, 'Uno o más servicios de los catálogos no existen o están inactivos');
    }
    return {
 ...
 */
export async function listStylists(_req, res, next) {
    try {
 ...
 */
export async function listStylistCatalogs(req, res, next) {
    try {
        const { id } = req.params;
 ...
 */
export async function listStylistCatalogServices(req, res, next) {
    try {
        ...
        const catalogIds = Array.isArray(stylist.catalogs)
            ? stylist.catalogs.map((c) => String(c))
            : [];
        if (!catalogIds.includes(String(catalogId))) {
            throw new ApiError(StatusCodes.FORBIDDEN, 'Ese catálogo no está asignado al estilista');
        }
 ...
 */
export async function createStylist(req, res, next) {
    try {
        const { nombre, apellido, cedula, telefono, genero, edad, email, password, catalogs } = req.body;
 ...
 */
export async function updateStylistServices(req, res, next) {
    try {
        const { id } = req.params;
        ...
        const { catalogObjectIds, serviceObjectIds } = await resolveCatalogsAndServices(catalogs);
        const updated = await UserModel.findOneAndUpdate({ _id: id, role: ROLES.ESTILISTA }, {
            $set: {
