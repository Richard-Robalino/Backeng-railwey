...
function parseEcStart(input) {
    const v = String(input);
    if (isDateOnly(v))
        return dayjs.tz(v, 'YYYY-MM-DD', TZ).startOf('day').toDate();
    const d = dayjs(v);
    if (!d.isValid())
        return new Date(''); // inválida
    return d.tz(TZ).toDate();
}
function parseEcEnd(input) {
    const v = String(input);
    if (isDateOnly(v))
        return dayjs.tz(v, 'YYYY-MM-DD', TZ).endOf('day').toDate();
    const d = dayjs(v);
    if (!d.isValid())
        return new Date('');
    return d.tz(TZ).toDate();
...
async function aggregateRevenueByStylist(start, end) {
    const pipeline = [
        {
            $addFields: {
                reportDate: { $ifNull: ['$fecha', '$createdAt'] },
...
async function aggregateBookingsByStatus(start, end) {
    const pipeline = [
...
async function aggregateRatingsByStylist(start, end) {
    const pipeline = [
...
async function aggregatePaidRevenueForStylist(stylistId, start, end) {
...
async function aggregateBookingStatsForStylist(stylistId, start, end) {
    const match = { estilistaId: new Types.ObjectId(stylistId), inicio: { $gte: start, $lte: end } };
    const pipeline = [
        { $match: match },
        {
            $facet: {
...
async function aggregateRatingsDetailForStylist(stylistId, start, end) {
    const match = { estilistaId: new Types.ObjectId(stylistId), createdAt: { $gte: start, $lte: end } };
    ...
    return {
        ...
        distribution: dist.map((d) => ({ stars: d._id, count: d.count })),
        latestComments: comments
    };
}
async function aggregateTopServicesForStylist(stylistId, start, end) {
    const sid = new Types.ObjectId(stylistId);
    const pipeline = [
        {
            $addFields: {
...
async function findLatestBookingNotesForStylist(stylistId, start, end) {
    const match = {
...
async function aggregatePeakForStylist(stylistId, start, end) {
    const sid = new Types.ObjectId(stylistId);
...
export async function summaryReports(req, res, next) {
    try {
        ...
        const [revenueByDay, revenueByStylist, topServices, bookingsByStatus, ratingsByStylist] = await Promise.all([
        ...
        const totalPaidBookings = revenueByStylist.reduce((acc, r) => acc + (r.bookingsCount || 0), 0);
        res.json({
...
export async function stylistRevenueReport(req, res, next) {
    try {
        const { from, to } = req.query;
        const { start, end, label } = normalizeFromTo(String(from), String(to));
...
export async function stylistsReport(req, res, next) {
    try {
        const { from, to, stylistId } = req.query;
        const { start, end, label } = normalizeFromTo(String(from), String(to));
        const userFilter = { role: ROLES.ESTILISTA };
        if (stylistId)
            userFilter._id = new Types.ObjectId(String(stylistId));
        ...
        const stylists = await (await import('../../models/User.js')).UserModel
            ...
            .lean();
        if (stylistId && !stylists.length)
            throw new ApiError(StatusCodes.NOT_FOUND, 'Estilista no encontrado');
        const reports = await Promise.all(stylists.map(async (s) => {
...
// ================== PDF HELPERS ==================
function drawBarChart(doc, title, data, maxBars = 8) {
    if (!data.length) {
...
function drawPieLikeSummary(doc, title, data) {
    doc.addPage();
    doc.fontSize(16).text(title, { underline: true });
    doc.moveDown();
...
// ================== PDF GENERAL (LOCAL) ==================
export async function downloadReportsPdf(req, res, next) {
    try {
        const { from, to } = req.query;
        const { start, end, label } = normalizeFromTo(String(from), String(to));
        const [revenueByDay, revenueByStylist, topServices, bookingsByStatus, ratingsByStylist] = await Promise.all([
        ...
        // Gráficos
        drawBarChart(doc, 'Gráfico - Ingresos por día', revenueByDay.map(r => ({ label: String(r.day).slice(5), value: Number(r.total || 0) })), 10);
        drawBarChart(doc, 'Gráfico - Ingresos por estilista (Top 8)', revenueByStylist.map(r => ({ label: r.stylistName, value: Number(r.totalRevenue || 0) })), 8);
        drawPieLikeSummary(doc, 'Distribución de citas por estado', bookingsByStatus.map(r => ({ label: r._id || 'SIN_ESTADO', value: r.count })));
        doc.addPage();
        doc.fontSize(16).text('Rating promedio por estilista', { underline: true });
        doc.moveDown();
        ratingsByStylist.forEach(r => {
...
// ================== PDF POR ESTILISTAS ==================
export async function downloadStylistsReportPdf(req, res, next) {
    try {
        const { from, to, stylistId } = req.query;
        const { start, end, label } = normalizeFromTo(String(from), String(to));
        ...
        doc.fontSize(10).text('Estilista | Ingresos | Citas pagadas | Ticket prom. | Rating | Reseñas | Clientes únicos | Cancelación% | Pico', { underline: true });
        doc.moveDown(0.5);
        sorted.forEach(r => {
            doc.fontSize(10).text(`${r.stylist.name} | $${r.paid.totalRevenue.toFixed(2)} | ${r.paid.paidBookings} | $${r.avgTicket.toFixed(2)} | ` +
        ...
        drawBarChart(doc, 'Top ingresos por estilista (Top 8)', sorted.slice(0, 8).map(r => ({ label: r.stylist.name, value: Number(r.paid.totalRevenue || 0) })), 8);
        ...
        if (includeDetails) {
            for (const r of sorted) {
                ...
                r.ratings.distribution.forEach(d => {
                ...
                if (!r.ratings.latestComments.length) {
                    ...
                    r.ratings.latestComments.forEach(c => {
...
function enforceStylistScope(req) {
...
export async function myStylistReport(req, res, next) {
    try {
...
export async function downloadMyStylistReportPdf(req, res, next) {
    try {
        req.query.stylistId = String(req.user.id);
