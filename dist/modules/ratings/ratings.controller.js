...
export async function createRating(req, res, next) {
    try {
        ...
        if (!booking)
            throw new ApiError(StatusCodes.NOT_FOUND, 'Reserva no existe');
        if (booking.clienteId.toString() !== userId)
        ...
        if (exists)
            throw new ApiError(StatusCodes.CONFLICT, 'Ya calificaste esta reserva');
...
export async function listMyRatingsClient(req, res, next) {
    try {
        const userId = req.user.id;
        const { dateFrom, dateTo, minStars, maxStars } = req.query;
        const page = Number(req.query.page ?? 1);
        const limit = Number(req.query.limit ?? 20);
        const q = { clienteId: userId };
        if (minStars || maxStars) {
            q.estrellas = {};
            if (minStars)
                q.estrellas.$gte = Number(minStars);
            if (maxStars)
                q.estrellas.$lte = Number(maxStars);
        }
        if (dateFrom || dateTo) {
            q.createdAt = {};
            if (dateFrom)
                q.createdAt.$gte = new Date(String(dateFrom));
            if (dateTo)
                q.createdAt.$lte = new Date(String(dateTo));
        }
...
export async function listMyRatingsStylist(req, res, next) {
    try {
        const stylistId = req.user.id;
        const { dateFrom, dateTo, minStars, maxStars, withStats } = req.query;
        const page = Number(req.query.page ?? 1);
        const limit = Number(req.query.limit ?? 20);
        const q = { estilistaId: stylistId };
        if (minStars || maxStars) {
            q.estrellas = {};
            if (minStars)
                q.estrellas.$gte = Number(minStars);
...
export async function listRatingsByStylistId(req, res, next) {
    try {
...
export async function getRatingById(req, res, next) {
    try {
        const r = await RatingModel.findById(req.params.id);
        if (!r)
            throw new ApiError(StatusCodes.NOT_FOUND, 'Calificación no encontrada');
        const u = req.user;
        if (u.role === ROLES.ADMIN || u.role === ROLES.GERENTE)
            return res.json(r);
        if (u.role === ROLES.CLIENTE && r.clienteId.toString() === u.id)
            return res.json(r);
        if (u.role === ROLES.ESTILISTA && r.estilistaId.toString() === u.id)
            return res.json(r);
        throw new ApiError(StatusCodes.FORBIDDEN, 'No autorizado');
    }
    catch (err) {
