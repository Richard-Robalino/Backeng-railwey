...
function buildDateTimeForSlotEC(dateStr, minutesFromMidnight) {
 ...
 * date: "YYYY-MM-DD"
 */
export async function computeAvailability(dateStr, serviceId, stylistId) {
    ...
    const slots = await ServiceSlotModel.find(slotFilter)
        .populate('stylist', 'nombre apellido')
        .sort({ startMin: 1 });
    if (!slots.length)
        return [];
    const stylistIds = Array.from(new Set(slots.map(s => s.stylist?._id?.toString()).filter(Boolean)));
    ...
    const busyByStylist = new Map();
    for (const b of busy) {
        const key = b.estilistaId.toString();
        if (!busyByStylist.has(key))
            busyByStylist.set(key, []);
