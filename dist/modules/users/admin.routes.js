...
router.patch('/:id/status', requireRoles(ROLES.ADMIN, ROLES.GERENTE), validateBody(updateUserStatusSchema), setUserActiveStatus);
// (Opcional) Alias semánticos
router.patch('/:id/activate', requireRoles(ROLES.ADMIN, ROLES.GERENTE), activateUser);
router.patch('/:id/deactivate', requireRoles(ROLES.ADMIN, ROLES.GERENTE), deactivateUser);
