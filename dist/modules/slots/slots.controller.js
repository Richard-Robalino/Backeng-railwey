...
async function validateStylistAndService(stylistId, serviceId) {
    ...
    if (!catExists) {
        throw new ApiError(StatusCodes.FORBIDDEN, 'El servicio no está asociado a este estilista');
    }
    // Validar duración del servicio
    const duracion = service.duracionMin;
    if (!duracion || duracion <= 0) {
        throw new ApiError(StatusCodes.BAD_REQUEST, 'El servicio no tiene duración válida');
    }
    if (duracion % 30 !== 0) {
        throw new ApiError(StatusCodes.BAD_REQUEST, 'La duración del servicio debe ser múltiplo de 30 minutos para crear horarios disponibles');
    }
    return { stylist, service, duracion };
}
async function ensureNoOverlap(stylistId, dayOfWeek, startMin, endMin, excludeId) {
    const filter = {
        stylist: stylistId,
        dayOfWeek,
        isActive: true,
    ...
    if (overlap) {
        throw new ApiError(StatusCodes.CONFLICT, 'Ya existe un horario disponible para este estilista que se cruza con ese rango');
    }
}
// ---------------- Controllers ----------------
/**
 ...
 */
export async function createDaySlots(req, res, next) {
    try {
        ...
        if (!slotsToInsert.length) {
            throw new ApiError(StatusCodes.BAD_REQUEST, 'Con ese rango de horas y esa duración no se pudo generar ningún horario');
        }
...
export async function listSlots(req, res, next) {
    try {
        const { stylistId, serviceId, dayOfWeek, onlyActive } = req.query;
...
export async function updateSlot(req, res, next) {
    try {
        ...
        if (minutes !== duracion) {
            throw new ApiError(StatusCodes.BAD_REQUEST, `La duración del horario (${minutes} min) debe coincidir con la duración del servicio (${duracion} min)`);
        }
